...
TAG = 'quota-cache-preview-1'
VERSIONS = {'quota-cache':'0.1.0','account-health-pushover':'0.4.1','reset-priority':'0.1.5'}
LIBRARIES = {'quota-cache':'dist/quota-cache.so','account-health-pushover':'dist/account-health-pushover.so','reset-priority':'reset-priority.so'}
...
def main():
    ...
    for plugin,version in VERSIONS.items():
        ...
        with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_DEFLATED) as z:
            for name,raw,mode in [(plugin+'.so',library,0o755),('LICENSE',(ROOT/'plugins'/plugin/'LICENSE').read_bytes(),0o644)]:
        ...
        with zipfile.ZipFile(archive) as z:
            assert z.namelist() == [plugin+'.so','LICENSE']
