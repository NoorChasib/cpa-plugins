...
class ReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "registry.json").write_text(json.dumps(release.source_registry("0.1.0")))
        elf = bytearray(64)
        ...
        self.bundle = release.package(self.dist, "0.1.0", self.root)
        release.checksums(self.dist, "0.1.0")

    def verify(self):
        release.verify(self.dist, "0.1.0", self.root)
    ...
    def test_exact_bundle_and_public_registry(self):
        ...
        self.assertEqual(artifact["url"], "https://github.com/NoorChasib/cpa-plugin-token-usage/releases/download/v0.1.0/token-usage_0.1.0_linux_amd64.zip")
    ...
    def test_checksums_never_rewrite_committed_source_registry(self):
        ...
        self.assertEqual(json.loads((ROOT / "registry.json").read_text()), release.source_registry("0.1.0"))

    def test_reproducible_zip_for_identical_inputs(self):
        first = self.bundle.read_bytes()
        release.package(self.dist, "0.1.0", self.root)
        self.assertEqual(first, self.bundle.read_bytes())

    def test_reject_unsupported_release(self):
        for version in ("v0.1.0", "../0.1.0", "0.2.0", "0.1.0\n", ""):
    ...
    def test_reject_fixture_build(self):
        for tags in (b"nativefixture", b"other,nativefixture"):
    ...
    def test_reject_extra_platform_archive(self):
        (self.dist / "token-usage_0.1.0_linux_arm64.zip").write_bytes(b"unclaimed")
    ...
    def test_reject_wrong_registry_urls_and_metadata(self):
        ...
        for key, value in (("url", "https://127.0.0.1:8765/unpublished.zip"),
                           ...
                           ("url", release.release_base("0.1.0").replace("v0.1.0", "v0.2.0") + "/" + self.bundle.name),
                           ("url", release.release_base("0.1.0").replace("https://", "http://") + "/" + self.bundle.name),
    ...
    def test_reject_changed_committed_registry(self):
        registry = release.source_registry("0.1.0")
...
class WorkflowTests(unittest.TestCase):
    def test_read_only_ci_and_required_smoke(self):
        ...
        self.assertIn("run: make smoke", workflow)
    ...
    def test_release_is_guarded_and_ordered(self):
        ...
        self.assertIn("tags: [v0.1.0]", workflow)
        self.assertIn("github.repository == 'NoorChasib/cpa-plugin-token-usage'", workflow)
        self.assertIn("github.event_name == 'push'", workflow)
        self.assertIn("github.ref == 'refs/tags/v0.1.0'", workflow)
        ...
        for forbidden in ("pull_request:", "pull_request_target:", "workflow_dispatch:", "secrets.", "continue-on-error", "always()", "--clobber", "package-current", "git push"):
            self.assertNotIn(forbidden, workflow)
        gates = ['make ci VERSION=', 'make smoke | tee', 'shutil.copyfile(work / "token-usage-production.so"',
                 'make package-tested checksums verify-release', 'python3 scripts/verify-cpa-package.py',
    ...
    def test_release_guard_rejects_wrong_repository_event_ref_and_version(self):
        ...
        environment = {**os.environ, "GITHUB_REPOSITORY": "NoorChasib/cpa-plugin-token-usage",
                       ...
                       "GITHUB_REF": "refs/tags/v0.1.0", "RELEASE_TAG": "v0.1.0", "RELEASE_VERSION": "0.1.0"}
    ...
    def test_existing_release_and_api_errors_fail_closed(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            env = {**os.environ, "PATH": directory + ":" + os.environ["PATH"], "RUNNER_TEMP": directory,
                   "RELEASE_REPO": "NoorChasib/cpa-plugin-token-usage", "RELEASE_TAG": "v0.1.0"}
    ...
    def test_draft_asset_validation_rejects_published_or_partial_release(self):
        ...
        valid = {"isDraft": True, "tagName": "v0.1.0", "assets": [
            {"name": "checksums.txt"}, {"name": "registry.json"}, {"name": release.archive_name("0.1.0")} ]}
