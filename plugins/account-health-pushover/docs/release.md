# Release account-health-pushover
...
git tag account-health-pushover/v0.4.3
git push origin account-health-pushover/v0.4.3
```
...
Run the Makefile checks from `plugins/account-health-pushover`. The authoritative full release checks live in root `scripts/verify-plugin-release.sh`, followed by `scripts/quota-cache-smoke.py --candidate account-health-pushover`. The root workflow executes both before packaging the exact tested bytes.
