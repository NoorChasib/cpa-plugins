...
Run the Makefile checks from `plugins/reset-priority`. The authoritative full release checks live in root `scripts/verify-plugin-release.sh`, followed by `scripts/quota-cache-smoke.py --candidate reset-priority`. The root workflow executes both before packaging the exact tested bytes.
