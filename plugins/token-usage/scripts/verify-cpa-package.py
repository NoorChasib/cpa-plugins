...
const releaseBase = "https://github.com/NoorChasib/cpa-plugins/releases/download/v0.1.2"
const apiBase = "https://api.github.com/repos/NoorChasib/cpa-plugins/releases/"
const archiveName = "token-usage_0.1.2_linux_amd64.zip"
...
func (l local) Do(r *http.Request) (*http.Response, error) {
    ...
    case apiBase + "latest", apiBase + "tags/v0.1.2":
        fixture := store.Release{TagName: "v0.1.2"}
...
func main() {
    ...
    for index, registryURL := range []string{sourceURL, releaseBase + "/registry.json"} {
        ...
        if plugin.ID != "token-usage" || plugin.Version != "0.1.2" { panic("wrong identity") }
        ...
        if installed.Skipped || installed.Version != "0.1.2" || installed.InstallType != expectedType { panic("wrong initial install") }
        ...
        if index == 0 {
            fixed, err := client.InstallVersion(ctx, plugin, "v0.1.2", "0.1.2", options)
            if err != nil || !fixed.Skipped { panic("exact-tag installation failed") }
            if _, err = client.InstallVersion(ctx, plugin, "v0.1.2", "0.2.0", options); err == nil { panic("wrong release version accepted") }
...
def main():
    ...
    release.verify(args.dist, "0.1.2")
