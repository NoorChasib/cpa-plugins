...
// terminalShutdownOnce guards the irreversible, process-global terminal
// shutdown transition so the test can run under -count>1: only the first
// run performs and asserts the transition; later runs skip.
var terminalShutdownOnce sync.Once
...
func TestTerminalNativeShutdownGatesInitAndCallsButNotBufferRelease(t *testing.T) {
	ran := false
	terminalShutdownOnce.Do(func() { ran = true })
	if !ran {
		t.Skip("terminal native shutdown is process-global and already ran in this process")
	}
