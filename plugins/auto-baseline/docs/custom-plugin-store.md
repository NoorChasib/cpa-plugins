# Install auto-baseline from the Plugin Store
...
Select **auto-baseline** and install it. Configure the options in [the quick start](../README.md), then follow any restart prompt. If you already use the identical `preview/registry.json` alias, keep that source URL so CPA's installed source identity stays the same.
