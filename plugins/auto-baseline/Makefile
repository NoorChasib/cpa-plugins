# Makefile for cpa-plugin-auto-baseline.
#
...
#
# On Linux, `make build` produces auto-baseline.so in the repository root.
...
## build: build the native CPA shared library (auto-baseline.so on Linux)
build:
...
## package: build and produce dist/auto-baseline_$(VERSION)_$(GOOS)_$(GOARCH).zip + .sha256
