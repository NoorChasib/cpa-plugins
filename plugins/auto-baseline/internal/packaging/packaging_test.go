...
func TestArchiveNamesMatchStoreExpectations(t *testing.T) {
	...
	want := map[[2]string]string{
		{"linux", "amd64"}:   "auto-baseline_" + plugin.PluginVersion + "_linux_amd64.zip",
		{"linux", "arm64"}:   "auto-baseline_" + plugin.PluginVersion + "_linux_arm64.zip",
		{"darwin", "amd64"}:  "auto-baseline_" + plugin.PluginVersion + "_darwin_amd64.zip",
		{"darwin", "arm64"}:  "auto-baseline_" + plugin.PluginVersion + "_darwin_arm64.zip",
		{"windows", "amd64"}: "auto-baseline_" + plugin.PluginVersion + "_windows_amd64.zip",
	}
...
func TestLibraryBasenames(t *testing.T) {
	...
	cases := map[string]string{"linux": "auto-baseline.so", "darwin": "auto-baseline.dylib", "windows": "auto-baseline.dll"}
...
func TestParseChecksums(t *testing.T) {
	sums, err := ParseChecksums(
		"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef  auto-baseline_0.1.0_linux_amd64.zip\n" +
			"ABCDEF0123456789abcdef0123456789abcdef0123456789abcdef0123456789  auto-baseline_0.1.0_darwin_arm64.zip\n\n")
	if err != nil {
	...
	if sums["auto-baseline_0.1.0_linux_amd64.zip"] == "" {
		t.Errorf("linux archive checksum missing")
	}

	// Path-prefixed names break the host's bare-asset-name lookup.
	if _, err := ParseChecksums("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef  ./auto-baseline_0.1.0_linux_amd64.zip"); err == nil {
...
func TestValidateRegistryShape(t *testing.T) {
	...
	if reg.Plugins[0].ID != "auto-baseline" {
...
func TestFormatChecksumsRoundTrip(t *testing.T) {
	in := map[string]string{
		"auto-baseline_0.1.0_linux_amd64.zip":  strings.Repeat("ab", 32),
		"auto-baseline_0.1.0_darwin_arm64.zip": strings.Repeat("CD", 32),
	}
	...
	if len(out) != 2 || out["auto-baseline_0.1.0_darwin_arm64.zip"] != strings.Repeat("cd", 32) {
...
func TestValidateArchiveLayout(t *testing.T) {
	...
	cases := map[string]map[string]string{
		...
		"versioned library": {
			"auto-baseline-v0.1.0.so": "elf",
			"LICENSE":                 "MIT",
		},
		"nested library": {
			"lib/auto-baseline.so": "elf",
			"LICENSE":              "MIT",
		},
	...
	duplicate := buildZipEntries(t, [][2]string{
		{"auto-baseline.so", "elf-a"},
		{"auto-baseline.so", "elf-b"},
		{"LICENSE", "MIT"},
	})
...
func TestRepositoryRegistryJSON(t *testing.T) {
	...
	raw := mustReadFile(t, path) // the file is a shipped release deliverable
