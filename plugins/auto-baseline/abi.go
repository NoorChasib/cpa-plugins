...
extern int autoBaselinePluginCall(char*, uint8_t*, size_t, cliproxy_buffer*);
extern void autoBaselinePluginFree(void*, size_t);
extern void autoBaselinePluginShutdown(void);

static const cliproxy_host_api* auto_baseline_host;

static void auto_baseline_store_host(const cliproxy_host_api* host) {
	auto_baseline_host = host;
}

static void auto_baseline_clear_host(void) {
	auto_baseline_host = NULL;
}

static int auto_baseline_call_host(const char* method, const uint8_t* request, size_t request_len, cliproxy_buffer* response) {
	if (auto_baseline_host == NULL || auto_baseline_host->call == NULL) {
		return 1;
	}
	return auto_baseline_host->call(auto_baseline_host->host_ctx, method, request, request_len, response);
}

static void auto_baseline_free_host_buffer(void* ptr, size_t len) {
	if (auto_baseline_host != NULL && auto_baseline_host->free_buffer != NULL && ptr != NULL) {
		auto_baseline_host->free_buffer(ptr, len);
	}
}
*/
import "C"
...
func (cgoHostCaller) Call(method string, request []byte) ([]byte, error) {
	...
	code := C.auto_baseline_call_host(cMethod, requestPtr, C.size_t(len(request)), &response)

	raw, err := consumeHostResponse(response.ptr, response.len, func() {
		C.auto_baseline_free_host_buffer(response.ptr, response.len)
	})
	if err != nil {
...
func cliproxy_plugin_init(host *C.cliproxy_host_api, pluginAPI *C.cliproxy_plugin_api) C.int {
	...
	C.auto_baseline_store_host(host)
	pluginAPI.abi_version = C.uint32_t(hostapi.ABIVersion)
	pluginAPI.call = C.cliproxy_plugin_call_fn(C.autoBaselinePluginCall)
	pluginAPI.free_buffer = C.cliproxy_plugin_free_fn(C.autoBaselinePluginFree)
	pluginAPI.shutdown = C.cliproxy_plugin_shutdown_fn(C.autoBaselinePluginShutdown)
	return 0
}

//export autoBaselinePluginCall
func autoBaselinePluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	if !beginNativeCall() {
		if response != nil {
			response.ptr = nil
			response.len = 0
...
//export autoBaselinePluginFree
func autoBaselinePluginFree(ptr unsafe.Pointer, length C.size_t) {
...
//export autoBaselinePluginShutdown
func autoBaselinePluginShutdown() {
