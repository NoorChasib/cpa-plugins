...
Maintained contract for **Token Usage 0.1.1**, updated 2026-09-09. Published v0.1.0 remains unchanged. The pinned upstream source audit and historical executor results below are not a deployed-console verification or a promise of billing-grade accuracy. All required final post-review local gates passed; [verification-sidebar.md](verification-sidebar.md) records the completed sidebar/default-storage/native/package evidence, exact local artifact hashes and material limitations. The linked artifact hashes describe historical local prepublication validation, not hosted release checksums.
...
| Candidate official-image install-to-startup | **Passed final local acceptance:** `eceasy/cli-proxy-api@sha256:3990e4de484ac5caac80164ee3a60d0ba521320dcda193a2ef71a5ad2e2c768b`, Linux amd64, Debian bookworm/glibc **2.36**, cwd `/CLIProxyAPI`; actual store handler/default-volume/native-page/restart. Final exact tested-library/package/frozen-notice verification also passed. |
...
- Schema 6 returns JSON, not HTML-safe text. The sidebar renders untrusted strings with text nodes rather than HTML sinks. The historical native fixture verifies `<model>&literal` survives unchanged through CPA; the candidate browser suite separately checks inert hostile model text.
...
### Private management routes and the public sidebar

The route namespace is `/v0/management/plugins/token-usage`. The candidate registers private `/plugins/token-usage/status`, `/plugins/token-usage/summary` and `/plugins/token-usage/models`. **Every private GET's `Menu` must remain empty.** A nonempty menu on a management GET makes CPA treat it as a legacy **public resource**.

Exactly one separate resource `/status` receives menu **Token Usage**, served at `/v0/resource/plugins/token-usage/status`. It is fixed, data-free HTML/CSS/JavaScript, not public operational data. The page automatically calls the three private APIs using a compatible remembered same-origin console session. CPA authenticates management fallback routes before dispatch but does not authenticate resources. Cookie credentials alone are insufficient; the page sends the existing management key in the authorization header. Exact private/resource dispatch and the data-free resource boundary are essential.

The historical v0.1.0 native fixtures rejected missing/invalid management credentials (401/403), returned 404 on both tested resource spellings and exposed readable authenticated status. That was real HTTP evidence **for the no-UI version**, not the candidate's expected public result. Fresh candidate native and official-image acceptance now passed the updated contract: exact public sidebar GET returns 200 with fixed/no-private-data bytes; unknown/resource-spelled private paths remain nonprivate; all private routes reject missing/wrong auth. The unavailable-storage case retains registration/resource and returns private 503 without fabricated coverage/counters. These are completed integration results, not yet final post-review results; see [candidate verification](verification-sidebar.md).

The audited console uses modern `cli-proxy-auth` with remembered key and optional `enc::v1::` host/user-agent XOR obfuscation. Present modern state is authoritative even after logout/error/remember-off; legacy-only fallback requires modern absence, a logged-in marker and matching normalized base. Origin/effective-port/prefix matching permits URL default-port and terminal management/slash normalization, not relaxed scheme/prefix binding. Parent-memory-only sessions are unavailable. Cross-origin framing is intentionally denied by `frame-ancestors 'self'`; no new login, key-in-URL/message/log bridge or CORS widening is supplied. Same-origin scripts remain mutually trusted. See [exact console/reference source audit](sidebar-audit.md).
...
The **original Phase A bootstrap on 2026-09-09** reported `storage: none_spike_only` and did not implement persistence. That is historical evidence, not the current runtime. The retained harness now uses the SQLite-backed plug-in and an explicit fresh private `database-path` fixture override. The candidate itself no longer requires that override: it defaults beneath captured cwd. The raw-record probe still tests the upstream contract, not durability; [separate historical production acceptance](#current-persistent-acceptance) verified committed history and restart behavior.
...
The **historical v0.1.0** production acceptance run on **2026-09-09** used the ordinary library, not the raw-record probe. This section preserves that accounting evidence; the 0.1.1 sidebar/default-storage native rerun is tracked separately in [verification-sidebar.md](verification-sidebar.md). [`scripts/native/production.py`](../scripts/native/production.py) exercised real pinned CPA executors through committed SQLite rows and authenticated `/status`, `/summary` and `/models`, then cleanly restarted against the same database. Both statistics settings passed, including provider/model separation, equal-looking repeat observations, exact counters above 2^53, private-route authentication and sensitive-canary checks. The direct ABI probe separately verified failed nonzero counters and exact history after quiesce/reopen/native reinitialization.

The **final 0.1.1 candidate native acceptance** passed at `/tmp/token-usage-smoke.ubZRD0hP`, including both ABI probes, 30 retained Phase A and 38 production HTTP executions with the updated resource/auth contract. Final official-image store-handler/default-volume/restart acceptance passed at `/tmp/token-usage-store-o7yk257l`, including the optionally/manually executed checksum-locked immutable v0.1.0 registration-failure control (not a CI gate), candidate unavailable-storage behavior, actual HTTP headers/positive CSP enforcement and a real browser rendering 1 then 2 events through the native page, pinned-codec synthetic remembered session and actual CPA JSON. This was not full console login/sidebar-navigation testing. Final `make ci` passed 39 Python contracts and Go/vet/race/build checks; the mandatory browser suite passed 71 scenarios. **Final packaging, both SDK installers and exact tested-library/frozen-notice validation passed**, with local artifacts saved under `dist/0.1.1/`. See [candidate verification](verification-sidebar.md) and [development](development.md) for exact hashes and evidence. All required local gates are complete, but no publication, hosted workflow run or operator installation is claimed; historical telemetry evidence alone was not used as current acceptance proof.
...
6. Retain empty private `Menu`, exactly one fixed data-free sidebar resource, strict route dispatch, no raw error reflection, and lifecycle exclusion. Private statistics must never become a public resource. `Shutdown()` must synchronously join every plug-in-owned worker before the native ABI returns.
