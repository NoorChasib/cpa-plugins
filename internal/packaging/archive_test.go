...
func TestBuildReleaseArchiveLayoutAndSidecar(t *testing.T) {
	...
	wantName := "auto-baseline_" + plugin.PluginVersion + "_linux_amd64.zip"
	...
	raw := mustReadFile(t, artifact.ArchivePath)
	...
	if !names["auto-baseline.so"] || !names["LICENSE"] || len(names) != 2 {
		t.Errorf("archive entries = %v, want exactly auto-baseline.so and LICENSE", names)
	}

	// Sidecar: bare filename, parseable, matching digest.
	sidecarRaw := mustReadFile(t, artifact.ChecksumPath)
...
func TestBuildReleaseArchiveDeterministic(t *testing.T) {
	...
	rawA := mustReadFile(t, first.ArchivePath)
	rawB := mustReadFile(t, second.ArchivePath)
...
func TestVerifyPublishedReleaseDirFailures(t *testing.T) {
	archive := "auto-baseline_" + plugin.PluginVersion + "_linux_amd64.zip"
	...
	t.Run("digest mismatch", func(t *testing.T) {
		out, checksumsPath := buildPublishedRelease(t)
		raw := mustReadFile(t, checksumsPath)
		flipped := "0"
		if raw[0] == '0' {
			flipped = "1"
		}
...
func TestVerifyReleaseDirFailures(t *testing.T) {
	...
	t.Run("tampered archive fails checksum", func(t *testing.T) {
		...
		swapped := mustReadFile(t, filepath.Join(other, archive))
